...
// ── Caché de catálogo de modelos ──────────────────────────────────────────────

const CATALOG_CACHE_TTL_MS = 60 * 60 * 1000; // 1 hora
...
const buildCatalogCacheKey = (apiKey: string, projectId: string | undefined, versions: GeminiApiVersion[]): string =>
    `${apiKey.slice(-8)}::${projectId ?? 'default'}::${versions.join(',')}`;

const getCatalogCache = (key: string): GeminiModelSummary[] | undefined => {
    const entry = catalogCacheMap.get(key);
    if (!entry) return undefined;
    if (Date.now() > entry.expiresAt) {
        catalogCacheMap.delete(key);
        return undefined;
    }
    return entry.models;
};

const setCatalogCache = (key: string, models: GeminiModelSummary[]): void => {
    catalogCacheMap.set(key, { models, expiresAt: Date.now() + CATALOG_CACHE_TTL_MS });
};

export const clearCatalogCache = (): void => catalogCacheMap.clear();
...
export const listAccessibleGeminiModels = async ({
    ...
    const cacheKey = buildCatalogCacheKey(apiKey, projectId, versions);
    ...
    const result = Array.from(unique.values()).filter(model => model.supportsGenerateContent);
    setCatalogCache(cacheKey, result);
    return result;
};

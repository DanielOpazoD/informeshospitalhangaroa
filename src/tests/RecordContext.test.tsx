...
describe('RecordContext', () => {
    ...
    it('muestra toast de error cuando localStorage está lleno al guardar borrador', () => {
        const { restore } = installMockWindowStorage();
        storageRestorers.push(restore);

        // Make setItem throw a QuotaExceededError to simulate a full storage
        ...
        const { result } = renderHook(() => useRecordContext(), { wrapper: toastWrapper });

        act(() => {
            result.current.dispatchRecordCommand({ type: 'change_record_title', title: 'Test' });
            result.current.saveDraft('manual');
        });

        expect(showToast).toHaveBeenCalledWith(
            expect.stringContaining('almacenamiento local lleno'),
            'error',
        );
    });

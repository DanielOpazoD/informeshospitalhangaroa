...
describe('useKeyboardShortcuts', () => {
    ...
    const fireShortcut = (key: string, { meta = true, shift = false } = {}) => {
    ...
    it('calls onRestore on Ctrl/Cmd+Shift+N', () => {
        setup();
        fireShortcut('n', { shift: true });
        expect(onRestore).toHaveBeenCalledOnce();
    });

    it('does NOT call onRestore on Ctrl/Cmd+N without Shift (Firefox conflict)', () => {
        setup();
        fireShortcut('n');

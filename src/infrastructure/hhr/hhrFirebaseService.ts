...
/** @deprecated Use HhrCensusSnapshot from hhrTypes instead */
export type HhrHospitalCensusSnapshot = HhrCensusSnapshot;

let cachedServices: HhrFirebaseServices | null = null;

/** Fuerza la re-inicialización de los servicios Firebase en el próximo uso. */
export const resetHhrFirebaseServices = (): void => {
    cachedServices = null;
};
...
export const subscribeToHospitalCensusSnapshot = (
    dateKey: string,
    onSnapshotChange: (snapshot: HhrCensusSnapshot) => void,
    onError?: (error: Error) => void
): (() => void) => {
    const { db } = getHhrFirebaseServices();
    const snapshotRef = doc(db, FIRESTORE_COLLECTION.hospitals, getHhrHospitalId(), FIRESTORE_COLLECTION.dailyRecords, dateKey);
...
export const saveClinicalDocumentToHhr = async ({
    ...
    const hospitalId = getHhrHospitalId();
    if (!hospitalId) {
        throw new Error('Hospital ID no configurado. Revisa la variable VITE_HHR_FIREBASE_HOSPITAL_ID.');
    }
    if (!actor.uid) {
        throw new Error('El usuario no tiene sesión activa válida. Por favor inicia sesión nuevamente.');
    }
    ...
    if (!built.documentId) {
        throw new Error('No se pudo generar un ID de documento válido para guardar en HHR.');
    }

    await setDoc(
        doc(db, FIRESTORE_COLLECTION.hospitals, hospitalId, FIRESTORE_COLLECTION.clinicalDocuments, built.documentId),
        built.payload,
        { merge: true }
    );

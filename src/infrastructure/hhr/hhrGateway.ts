...
export interface HhrGateway {
    ...
    subscribeCensus: (
        dateKey: string,
        onSnapshot: (snapshot: HhrCensusSnapshot) => void,
        onError?: (error: Error) => void,
    ) => () => void;
...
export const createHhrGateway = (): HhrGateway => ({
    ...
    subscribeCensus: (dateKey, onSnapshot, onError) => {
        appLogger.info('hhr-gateway', `subscribe_census :: iniciando suscripción para ${dateKey}`);
        return subscribeToHospitalCensusSnapshot(
            dateKey,
            onSnapshot,
            (error) => {
                appLogger.warn('hhr-gateway', `subscribe_census :: error en suscripción para ${dateKey}`, error);
                onError?.(error);
            },
        );
    },

...
import {
    createDriveGatewayImpl as createLegacyDriveGateway,
    type DriveGatewayImpl as LegacyDriveGateway,
    type DriveSearchFilters,
} from './driveGatewayImpl';
...
const logGatewayEvent = (label: string) => (event: { type: string; attempt: number; error?: string }) => {
    if (event.type === 'start' || event.type === 'success') return;
    const suffix = event.error ? ` (${event.error})` : '';
    appLogger.warn('drive-gateway', `${label} :: ${event.type} intento ${event.attempt}${suffix}`);
};

...
export const useClinicalRecord = ({
    ...
    const saveDraft = useCallback((reason: 'auto' | 'manual' | 'import', overrideRecord?: ClinicalRecord) => {
        ...
        const saved = draftPersistence.writeDraft({ timestamp, record: snapshot });
        if (!saved) {
            onToast(
                'No se pudo guardar el borrador: almacenamiento local lleno. ' +
                'Considera exportar el documento y limpiar datos del navegador.',
                'error',
            );
        }
